...
const nums3 = [1,50,1,2,40,2,2,3,10,3];
const k3 = 3;
...
function kMostFrequent(nums, k) {
    ...
    for(let i = 0; i < nums.length; i++){
        if(objList.hasOwnProperty(nums[i])){
            objList[nums[i]] += 1;
        }
        else objList[nums[i]] = 1;
    }
    // the next three lines are from google = (https://www.educative.io/answers/how-can-we-sort-a-dictionary-by-value-in-javascript)
    var items = Object.keys(objList).map((key) => { return [key, objList[key]]});
