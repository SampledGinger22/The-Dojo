...
//     for(let one in persons){
//         for(let habit in persons[one].habits){
//             if(persons[one].habits[habit] === badHabit){
//                 let name = persons[one].firstName + " " + persons[one].lastName;
//                 result.push(name);
//                 break;
//             }
//         }
...
function santasNaughtyListFunctional(persons, badHabit) {
    persons = persons.filter(person => person.habits.includes(badHabit) === true);
    let obj = persons.map(obj => obj.firstName + " " + obj.lastName);
